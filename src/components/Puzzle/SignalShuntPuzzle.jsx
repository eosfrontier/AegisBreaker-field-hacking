...
const SHAPE_MASKS = {
  circle: '/sci-fi-vector-circle.svg',
  square: '/sci-fi-vector-hashtag.svg',
  triangle: '/sci-fi-vector-triangle.svg',
  diamond: '/sci-fi-vector-cross.svg',
  hexagon: '/sci-fi-vector-hexagon.svg',
  star: '/sci-fi-vector-star.svg',
};
...
function generateRules(difficulty, seed) {
  ...
  const binCount = Math.min(5, difficulty <= 2 ? 3 : difficulty <= 4 ? 4 : 5);
  ...
  const buildNumberClause = () => {
    const options = [
      { fn: (p) => p.number % 2 === 0, desc: 'value is even' },
      { fn: (p) => p.number % 2 !== 0, desc: 'value is odd' },
      { fn: (p) => p.number % 3 === 0, desc: 'divisible by 3' },
      { fn: (p) => p.number % 4 === 0, desc: 'divisible by 4' },
      { fn: (p) => p.number > 5, desc: 'value > 5' },
      { fn: (p) => p.number < 4, desc: 'value < 4' },
      { fn: (p) => p.number >= 3 && p.number <= 7, desc: 'value 3-7' },
    ];
  ...
  const buildCompositeRule = (level) => {
    ...
    clausePool.push(() => {
      const subset = pickColorSubset(2, 3);
      return {
        fn: (p) => !subset.includes(p.color),
        desc: `color NOT in ${subset.join(', ')}`,
        tokens: [{ type: 'keyword', value: 'NOT ' }, ...toColorTokens(subset)],
      };
    });
    clausePool.push(() => {
      const subset = pickShapeSubset();
      return {
        fn: (p) => subset.includes(p.shape),
        desc: `shape in ${subset.join(', ')}`,
        tokens: [...toShapeTokens(subset)],
      };
    });
    clausePool.push(() => {
      ...
      return {
        ...
        tokens: [{ type: 'keyword', value: 'NOT ' }, ...toShapeTokens(subset)],
      };
    });
    clausePool.push(() => buildNumberClause());

    const clauseCount = level <= 3 ? 1 : level === 4 ? 2 : 3;
    const chosen = [];
    const usedTypes = new Set();
    while (chosen.length < clauseCount && clausePool.length > 0) {
      const idx = Math.floor(rng() * clausePool.length);
      const clauseBuilder = clausePool.splice(idx, 1)[0];
      const clause = clauseBuilder();
      if (usedTypes.has(clause.desc.split(' ')[0]) && clauseCount - chosen.length < 2) continue;
      chosen.push(clause);
      usedTypes.add(clause.desc.split(' ')[0]);
    }
  ...
  for (let i = 0; i < binCount; i++) {
    ...
    const useConditional = difficulty >= 5 && rng() < 0.35;
...
export default function SignalShuntPuzzle({ sessionId, layerId, layerData, onLocalPuzzleComplete }) {
  ...
  const packetCount = Math.min(24, 12 + difficulty * 2);
