...
$characterId = isset($_GET['characterId']) ? trim($_GET['characterId']) : '';
if ($characterId === '' || !preg_match('/^\d+$/', $characterId)) {
    http_response_code(400);
    echo json_encode(array('error' => 'invalid_characterId'));
    exit;
}
...
$url = 'https://api.eosfrontier.space/orthanc/v2/skills_character/?characterID=' . urlencode($characterId);
$headers = array(
    'accountID: ' . $joomlaId,
    'token: ' . $token
);
